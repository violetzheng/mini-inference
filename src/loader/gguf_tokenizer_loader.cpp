...
namespace mini_inference::loader
{
    ...
    namespace
    {
        ...
        BpeTokenizer build_gpt2_tokenizer(const GgufReader &reader)
        {
        ...
        SentencePieceTokenizer build_sentencepiece_tokenizer(const GgufReader &reader)
        {
            std::vector<std::string> pieces = reader.metadata_string_array("tokenizer.ggml.tokens");
            std::vector<float> scores = reader.metadata_float_array("tokenizer.ggml.scores");
    ...
    Tokenizer build_tokenizer(const GgufReader &reader)
    {
        ...
        {
            return build_gpt2_tokenizer(reader);
        }
        if (tokenizer_model == "llama")
        {
            return build_sentencepiece_tokenizer(reader);
        }
        throw std::invalid_argument("unsupported GGUF tokenizer model '" + tokenizer_model +
                                     "' (only 'gpt2' byte-level BPE and 'llama' SentencePiece are supported)");
    }
